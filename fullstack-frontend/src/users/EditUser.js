...
export default function EditUser() {
  ...
  useEffect(() => {
    loadUser();
  },[]);//array is empty means the code will run only onces
  ...
  const onInputChange = (e) => {
    setUser({ ...user, [e.target.name]: e.target.value }); //when a user types into an input field, the corresponding property in the user state is updated with the new value. 
  };
